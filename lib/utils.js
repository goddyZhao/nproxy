...
var utils = {
  ...
  setProxy: function(options) {
    if(options && options.host) {
      log.info('/!\\ Using proxy ' + options.host + ':' + options.port || '80');
      utils.proxy = options;
      utils.proxy.bypass = utils.proxy.bypass || ['127.0.0.1', 'localhost'];
    }
  },
  ...
  request: function(options, callback){
    ...
    if(!utils.proxy || utils.proxy.bypass.indexOf(requestUrl.hostname) !== -1) {
      ...
      if(requestUrl.protocol === 'https:') {
        ...
        if(proxy.protocol && proxy.protocol === 'https') {
          sender = https;
        }

        // proxy request with absolute path
        requestOptions = {
          host: proxy.host,
          port: proxy.port || 80,
          ...
          method: requestMethod,
          // Absolute URL
          path: requestUrl.protocol + '//' + requestUrl.hostname + requestUrl.path,
          rejectUnauthorized: false
        };
