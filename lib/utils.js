...
var utils = {
  ...
  processUrl: function(req){
    ...
    if(!parsedUrl.port && typeof port !== 'undefined'){
      parsedUrl.port = port;
    }

    if (!parsedUrl.port) {
      parsedUrl.port = req.type === 'https' ? 443 : 80;
    }
  ...
  request: function(options, callback){
    ...
    var agentInstance = agent.getByUrl(options.url);
    if (agentInstance) {
      requestOptions.agent = agentInstance;
    }
  ...
  getAvaliablePort: function (cb) {
    ...
    server.on('error', function (err) {
      log.debug(err);
      utils.getAvaliablePort(cb);
    });
  },

  getUserHomeDir: function () {
    var env = process.env;
    return env.HOME || env.HOMEPATH;
  },

  getCertDir: function () {
    var certDir = path.join(utils.getUserHomeDir(), '.nproxy_certs');
